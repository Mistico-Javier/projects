package dgtic.proyecto.security.jwt;
...
public class JWTTokenProvider {
    ...
    public String generateJwtToken(Authentication authentication, UsuarioDTO user) {
        Claims claims = Jwts.claims().setSubject("UNAM").setIssuer(user.getCorreo())
                .setAudience("JAVA");
        ...
        claims.put("issname", user.getCorreo()); //getNombre() + " " + user.getApellido_Paterno() + " " + user.getApellido_Materno()
    ...
    public String getCorreo(String token) { //getFullName
        key = Keys.hmacShaKeyFor(secret.getBytes());
